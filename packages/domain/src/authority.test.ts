...
describe("AuthorityRecord Domain Model", () => {
  ...
  describe("Validation", () => {
    it("accepts a well-formed input", () => {
      const result = validateAuthorityRecord(validRecordInput);
    ...
    it("rejects non-object inputs", () => {
      const result1 = validateAuthorityRecord(null);
      expect(result1.ok).toBe(false);
      if (!result1.ok) {
        expect(result1.error.code).toBe("INVALID_AUTHORITY_ID");
        expect(result1.error.field).toBe("authorityId");
        expect(result1.error.message).toContain(
          "Input must be a non-null object",
        );
      }

      const result2 = validateAuthorityRecord("not-an-object");
      expect(result2.ok).toBe(false);
      if (!result2.ok) {
        expect(result2.error.code).toBe("INVALID_AUTHORITY_ID");
        expect(result2.error.field).toBe("authorityId");
      }

      const result3 = validateAuthorityRecord([]);
      expect(result3.ok).toBe(false);
      if (!result3.ok) {
        expect(result3.error.code).toBe("INVALID_AUTHORITY_ID");
        expect(result3.error.field).toBe("authorityId");
      }
    });

    it("rejects empty or whitespace-only authorityId", () => {
      const result1 = validateAuthorityRecord({
        ...validRecordInput,
        authorityId: "",
      });
      expect(result1.ok).toBe(false);
      ...
      const result2 = validateAuthorityRecord({
        ...validRecordInput,
        authorityId: "   ",
      });
      expect(result2.ok).toBe(false);
    ...
    it("rejects invalid or non-UTC ISO-8601 validFrom", () => {
      ...
      for (const time of invalidTimes) {
        const result = validateAuthorityRecord({
          ...validRecordInput,
          validFrom: time,
        });
    ...
    it("accepts validTo equal to validFrom (zero duration, boundary case)", () => {
      ...
      const result = validateAuthorityRecord(zeroDurationInput);
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    it("preserves original values exactly without trimming or normalizing in output", () => {
      ...
      const result = validateAuthorityRecord(customInput);
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.authorityId).toBe("  auth-123  ");
  ...
  describe("Canonical Serialization", () => {
    it("serializes deterministically in exact alphabetical key order", () => {
      ...
      const serialized = serializeAuthorityRecord(record);

      // Verify exact alphabetical key order:
      // authorityId, scope, subjectId, validFrom, validTo
    ...
    it("repeated serialization produces byte-identical output", () => {
      ...
      const serialized1 = serializeAuthorityRecord(record);
      const serialized2 = serializeAuthorityRecord(record);
    ...
    it("equivalent records with different property insertion histories produce identical output", () => {
      ...
      recordB.authorityId = "auth-123";

      const serializedA = serializeAuthorityRecord(
        recordA as unknown as AuthorityRecord,
      );
      const serializedB = serializeAuthorityRecord(
        recordB as unknown as AuthorityRecord,
      );
    ...
    it("performs successful serialize -> JSON.parse -> validate round-trip", () => {
      ...
      const validationResult = validateAuthorityRecord(parsed);
  ...
  describe("Immutability", () => {
    it("expresses readonly contract at compile-time", () => {
      ...
      // @ts-expect-error AuthorityRecord properties are readonly
      record.authorityId = "new-id";
