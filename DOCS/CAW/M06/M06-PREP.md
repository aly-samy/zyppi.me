...
*   *Reconnaissance Finding:* The repository contains no reference pinning the codebase to a specific version of the GS1 standards.
*   *Status:* **EXTERNAL STANDARDS DECISION REQUIRED** — the repository does not identify or pin the governing GS1 authority.
...
*   *Status:* **CHAIR DECISION REQUIRED** — No support profile is formalized.
...
*   *Status:* **MISSING** — Table-driven or extensible AI grammar must be designed.
...
1.  **Initial Status Verification:**
    ...
    pnpm run test
    ```
    *Status:* **FAILED** (`PostgresError: connect ECONNREFUSED 127.0.0.1:5432`)
    *Reason:* No local PostgreSQL container service was running.
    *Finding:* The database dependency is a development prerequisite but was uninitialized in the sandbox.
...
3.  **Database Migration Application:**
    ...
    pnpm db:migrate
    ```
    *Result:* Successfully applied 1 migration: `001_initial_registry_schema.sql` (created tables: `referents`, `identities`, `evidence`, `policies`, `authorities`, `capabilities`, `standings`, `execution_receipts`, `schema_migrations`).
