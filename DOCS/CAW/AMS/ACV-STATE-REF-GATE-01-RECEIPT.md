...
8. **Relationship Normalization Rule:** Sorted ascending by `referentId`, then canonical JCS tie-break. No silent `?? []` repair.
9. **Standing Normalization Rule:** Sorted ascending by `standingId`, then canonical JCS tie-break. No silent `?? []` repair.
10. **Authority Normalization Rule:** Sorted ascending by `authorityId`, then canonical JCS tie-break. No silent `?? []` repair.
11. **Capability Normalization Rule:** Sorted ascending by `capabilityId`, then canonical JCS tie-break. No silent `?? []` repair.
12. **Applicable Policy Normalization Rule:** Sorted ascending by `policyId`, then `version`, then canonical JCS tie-break. No silent `?? []` repair.
...
27. **Restored EC Materialization Proof:** `evaluationCoordinate` is materialized when `tEInput` and governed `policyContext` are supplied; explicit `tEInput` is required for Packet-B assembly (rule B-0861-32 restored).
...
33. **Domain Tests Result:** PASS (23/23 tests in `packages/domain/src/acvState.test.ts` including CORR-ACV-STATE-REF-01 negative malformed-ACV tests; 438 total `@zyppi/domain` tests passed green).
34. **GS1 Tests Result:** PASS (29/29 tests in `apps/api/src/gs1/gs1CompositionBridge.test.ts`).
35. **Z-PROF Tests Result:** PASS (225/225 tests in `apps/api/src/zprof`).
36. **Full Relevant Workspace Test Result:** PASS (277 relevant domain/GS1/Z-PROF tests passed green).
