...
**Submitted GitHub Branch:** `docs/s13a-caw-m01-m04-migration-7207880810158899598`
**PR Number:** `#157`
**PR URL:** `https://github.com/aly-samy/zyppi.me/pull/157`
**Tested Head SHA:** `381e3421bda3a990c3cd6605a6302fa932ce2a41`
**Passing Remote CI:** `https://github.com/aly-samy/zyppi.me/actions/runs/35595133578` (CI Run #889)
...
- **Actual Input Versions Verified:**
  - Node.js: `v20.19.0` pinned in `package.json` (active local sandbox environment executed on Node `v22.22.1` with engine warning)
  - pnpm: `10.30.3`
  - TypeScript: `5.9.3`
  - Vitest: `4.1.10`
  - Prettier: `3.9.6`
...
- **Whole-Tree Comparison:** All 748 other tracked files in the repository remain 100% unchanged, including all 69 co-located S13B AMS exclusions, 17 S13C CCP files, and 2 S14 co-located files held in place untouched: `DOCS/CAW/CAW-Full-at-M03.md` (GENERATED-PACK) and `DOCS/CAW/_CAW-004-Repository-Map.md` (SUPERSEDED).
...
## 8. Transient Drift Disclosure & Cleanup

- **Observed Transient Drift Paths:** Running the Vitest unit test suite generated 5 transient artifact changes across two categories:
  - _Category 1: 3 Newly Generated Untracked Showcase Artifacts:_
    ...
    3. `DOCS/ZII/ZQE/evidence/fqr1/payload-b-showcase.svg`
       _(Generated by `tools/zqe/m06/showcase-print-helper.test.ts` during unit test execution)_
  - _Category 2: 2 Modified Tracked State Files:_ 4. `packages/testing/replay/receipts/latest.json` 5. `tools/zqe/mobile/android/app/src/androidTest/assets/manifest.json`
- **Cleanup Method & Execution:**
  - Category 1 (Untracked showcase files): Removed from the worktree using filesystem removal (`rm` / `git rm -f` / `git restore --staged`).
  - Category 2 (Modified tracked state files): Restored back to pristine baseline HEAD state using `git restore` / `git checkout`.
- **Drift Statements:**
...
1. `pnpm format:check` — PASSED (100% formatted via Prettier)
2. `pnpm lint` — PASSED (ESLint clean)
3. `pnpm exec tsc -b` — PASSED (TypeScript build clean)
...
5. `pnpm test` (local execution status & PostgreSQL dependency explanation):
   - Unmodified `pnpm test` command executes all Vitest suites in the repository.
   - When executed in the local sandbox container, 15 database integration tests across 4 test files (`infra/src/test/schema.test.ts`, `infra/src/test/migration.test.ts`, `apps/api/src/registry/postgres-registry.integration.test.ts`, `apps/api/src/registry/seed/seed.test.ts`) failed due to an un-started local PostgreSQL daemon (`connect ECONNREFUSED 127.0.0.1:5432`).
   - To verify non-database unit tests locally in the sandbox, an explicit test command with database exclusions was run: `pnpm test --exclude="**/*.integration.test.ts" --exclude="**/migration.test.ts" --exclude="**/schema.test.ts" --exclude="**/seed.test.ts"`, passing 61 test files (1,636 tests green).
6. `pnpm run ci` (local execution status):
   - Running `pnpm run ci` locally executes `pnpm format:check && pnpm lint && pnpm exec tsc -b && pnpm governance:validate && pnpm test`.
   - Locally, formatting, linting, type-checking, and governance validation pass 100%, while `pnpm test` stops at the PostgreSQL connection refusal as noted above.
7. `git diff --check` — PASSED (zero whitespace errors)
8. **Environment Limitation & Remote CI Proof:**
   - The active local sandbox environment executed under Node `v22.22.1` (emitting engine warning against pinned Node `v20.19.0`) and lacked an active PostgreSQL service.
   - Full workspace test compliance including PostgreSQL integration and pinned Node v20.19.0 environment was verified independently via remote GitHub Actions CI run #889 (`https://github.com/aly-samy/zyppi.me/actions/runs/35595133578`) on tested head `381e3421bda3a990c3cd6605a6302fa932ce2a41`.
