...
4. **Interleaving & RS:** Corrected to interleave _all_ data blocks, then _all_ ECC blocks. Replaced placeholders with exact GF(256) / BCH mathematics.
5. **Masking Flow:** Format information is now written _before_ penalty scoring to ensure accurate evaluation.
...
- _Exact Capacity:_ Terminator is omitted or truncated. Pad codewords are not needed.
- _Empty Input:_ Mode (4) + Count (8) + Term (4) = 16 bits (2 bytes). Remaining 42 bytes are `0xEC`, `0x11`...
...
_Note: Full polynomial division algorithm must be implemented using `gfMul` and `gfAdd` (which is simply XOR `^`)._
...
1. Clone the matrix.
2. **Apply Mask:** XOR data modules (state == `DATA`) with mask formula (e.g., Mask 0: `(row + col) % 2 == 0`). _Do not touch FUNCTION or RESERVED modules._
3. **Write Candidate Format Info:** Generate the 15-bit Format BCH for `maskId` and ECC Level M. Write these bits into the `RESERVED` format areas. _(Crucial: Format modules must be present to calculate penalty accurately)._
4. **Calculate Penalty:**
   - _Rule 1:_ Adjacent same-color modules in row/col (5+).
   - _Rule 2:_ 2x2 blocks of same color.
   - _Rule 3:_ 1:1:3:1:1 ratio patterns (finder-like).
   - _Rule 4:_ Deviation of dark module proportion from 50%.
5. Record score.
...
_Note: Full ZQE implementation will expand this matrix to include Micro QR (Annex A), Structured Append, and ECI designators._
