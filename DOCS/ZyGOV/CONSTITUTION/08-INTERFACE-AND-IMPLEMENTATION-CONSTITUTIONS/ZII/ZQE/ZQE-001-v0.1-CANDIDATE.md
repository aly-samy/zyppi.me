...
The FQR acceptance corpus includes:

| Fixture | Payload | Byte Length | Expected |
|---|---|---:|---|
| A | `HELLO ZYPPI` | 11 | SUCCESS |
| B | `https://id.gs1.org/01/09520123456788` | 36 | SUCCESS |
| C | `ZYPPI-FQR1-CAPACITY-BOUNDARY-0000000000001` | 42 | SUCCESS |
| D | `ZYPPI-FQR1-CAPACITY-BOUNDARY-0000000000001X` | 43 | `QR_CAPACITY_EXCEEDED` |
| E | `ZYPPI-FQR1-INTERIOR-TEST-2026` | 29 | SUCCESS |
...
FQR acceptance SHALL include a test-side strict verifier logically independent from the production encoder.

For FQR-1 it SHALL verify, at minimum:
...
Before this specification may advance from candidate to approved implementation authority, the FQR normative reviewer SHALL close the following standards-sensitive checks against the legitimate normative source:
...
Approval of this specification does not itself create package-admission authority.
...
This v0.1 specification is the principal candidate output of:
...
2. NVR-001 through NVR-011 are resolved;
...
When a future approved version of ZQE-001 becomes ACTIVE:
...
Approval of ZQE-001 does not itself authorize code.
